...
# --------------------------------------------------
# Device thresholds cache (from Resource Catalogue)
# --------------------------------------------------
class DeviceConfigCache:
    def __init__(self, catalogue_base_url, ttl_seconds=120):
    ...
    def get_thresholds(self, device_id):
        ...
        thresholds = self._fetch_from_catalogue(device_id) or {}
        self.cache[device_id] = {"ts": now_ts(), "thresholds": thresholds}
        return thresholds

    def _fetch_from_catalogue(self, device_id):
        try:
...
class MonitoringService:
    def __init__(self, cfg):
        ...
        self.cache = DeviceConfigCache(self.catalogue_base_url)

        self.pump_cooldown = int(cfg.get("pump_cooldown_sec", 180 * 60))
        self.last_pump_ts = {}

        # Sensors handled ONLY by prediction (no threshold check)
        self.predict_only_sensors = ["nitrate", "turbidity"]

        # Prediction service URL (from Service Catalogue)
        ...
        try:
            ...
            if r.status_code == 200:
                self.predict_base_url = r.json()["service"]["url"].rstrip("/")
    ...
    def on_agg_sensors(self, topic, payload):
        ...
        # Threshold check (ALL sensors except nitrate & turbidity)
        for sensor, rule in thresholds.items():
            if sensor in self.predict_only_sensors:
                continue
            ...
            if isinstance(val, (int, float)):
                if val < rule["min"] or val > rule["max"]:
                    alerts.append({
                        "device_id": device_id,
                        "level": "warning",
                        "message": f"Threshold violation: {sensor} ({val} outside [{rule['min']}, {rule['max']}])",
                        "ts": int(data.get("ts", now_ts())),
                        "meta": {"sensor": sensor, "value": val, "threshold": rule}
                    })

        # Prediction (ONLY nitrate + turbidity)
        ...
        for alert in alerts:
            self.mqtt.publish(f"aquarium/{device_id}/alerts", alert)
